...
ptn::Potion& ptn::Potion::operator* (const Potion& other) {
    ...
    for (int i = 0; i < other.ingredients_.size(); i++) {
        for (int j = 0; j < ingredients_.size(); j++) {
            if (ingredients_[j] == other.ingredients_[i]) {
                temp_ingredients_.emplace_back(other.ingredients_[i]);
            }
        }
    }
    ingredients_ = temp_ingredients_;
    ingredients_.emplace_back("Tail");
    ingredients_.emplace_back("Water");
    return *this;
}
...
const int ptn::Potion::get_type() { return type_; }
const vector<string> ptn::Potion::get_ingredients() {return ingredients_;}
...
void ptn::Potion::set_volume(int volume) {
     if (volume < relative_zero) {
        throw std::invalid_argument("Volume is less than 0.");
    }
    volume_ = volume;
}
void ptn::Potion::empty_the_glass() {
    if (volume_ <= relative_zero) {
        cout << "The glass is already empty." << endl;
    }
    else {
        volume_ = relative_zero;
        type_ = 0;
...
void ptn::Potion::drink() {
    if (volume_ <= relative_zero) {
        ...
        volume_ -= 0.1;
        cout << "You open a bottle and make a tiny sip." << endl;
    }
}
void ptn::Potion::change_base() {
    cout << "Please, choose the basic ingredient:" << endl;

    int t;
    cin >> t;
    if (!(1 <= t && t <= amount_of_bases)) {
