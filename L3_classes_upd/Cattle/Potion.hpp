...
const Base bases[amount_of_bases]{ //inline like pragma once
...
public:
    ...
    const int get_price();
    const int get_type();
    const vector<string> get_ingredients();
    ...
    void remove_ingredient(string what); // working-only functions
    void merge_ingredients(const Potion& other);
    void copy_base(const Potion& other);

    void empty_the_glass(); // user related functions(cout included)
    void drink();
    void change_base();  
};
}