...
short* get_i_with_zeros(short** table, short rows, short columns, short& k) { // ñîçäàòü ïðÿì òóò ìàñèâ zeros
...
void see(short** table, short rows, short columns) {
	for (short i = 0; i < rows; ++i) { // ïîñìîòðåòü, ÷òî òàì
		cout << '\n';
		for (short j = 0; j < columns; ++j) {
			cout << table[i][j] << ' ';
		}
...
int main() {
	...
	short** table = (short**)calloc(rows,sizeof(short*)); // ñòðîêè òàáëèèöû
	for (short i = 0; i < rows; i++) {
		table[i] = (short*)malloc(columns*sizeof(short)); // â êàæäîé ÿ÷åéêå ñòðîêè ïî ñòîëáöó
	}

	for (short i = 0; i < rows; i++) { // ïîëó÷àåì çíà÷åíèÿ
		for (short j = 0; j < columns; j++) {
			switch (i) {
			case 0: // çíà÷åíèÿ ïåðâîé ñòðîêè ïîëîæèòåëüíûå
	...
	table = (short**)realloc(table, new_rows * sizeof(short*)); //îáîçíà÷èòü áîëüøåå êîë-âî ñòðîê
	for (short i = 0; i < rows; ++i) {
		table[i] = (short*)realloc(table[i], new_columns * sizeof(short)); // äîáàâèòü êîëîíêè ê òåì ñòðîêàì, êîòîðûå óæå áûëè
	}
	for (short i = rows; i < new_rows; ++i) { // âûäåëèòü ìåñòî äëÿ êîëîíîê â ñîâñåì íîâûõ ñòðîêàõ 
	...
	for (short i = 0; i < new_rows; ++i) { // çàïîëíÿåì òàáëèöó
	...
	for (short i = 0; i < new_rows; ++i) { // ÷åêàåì
		cout << zeros[i] << ' ';
	}
	cout << '\n';
